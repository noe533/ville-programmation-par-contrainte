...
def draw_map(buildings, W, H, terrain, isRoad,
    ...
    # Terrain + routes (même orientation que MiniZinc / Houdini)
    for y in range(H):
        for x in range(W):
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Visualisation 2D debug")
    parser.add_argument("--buildings", "-b", required=True,
                        help="Fichier .dzn des bâtiments (solution MiniZinc)")
    parser.add_argument("--terrain", "-t", required=True,
                        help="Fichier .dzn du terrain")
    parser.add_argument("--output", "-o", required=False,
                        help="Chemin de l'image de sortie (.png)")
    parser.add_argument("--no-show", action="store_true",
                        help="Ne pas afficher la fenêtre matplotlib")

    args = parser.parse_args()

    buildings = read_buildings(args.buildings)
    W, H, terrain, isRoad = read_dzn_terrain(
        args.terrain,
        buildings=buildings,
        verbose=False
    )
    ...
    print(f"{len(conflicts)} bâtiments en conflit avec des routes")
