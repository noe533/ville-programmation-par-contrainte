...
MODEL=minizinc/models/placement.mzn
...
AFFICHAGE=visualisation/python/affichage2D.py
...
$(SOLUTION): $(TERRAIN) $(BUILDINGS) $(MODEL)
	$(MINIZINC) --solver $(SOLVER) --time-limit 120000 $(MODEL) > $(SOLUTION)

$(OUTPUT): $(SOLUTION) $(TERRAIN)
	mkdir -p $(dir $@)
	$(PYTHON) $(AFFICHAGE) \
	    --buildings $(SOLUTION) \
	    --terrain $(TERRAIN) \
